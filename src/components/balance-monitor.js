			...
			() => html`
					...
					${enemies.length > 0 ? 
						html`
							<table>
								...
								<tbody>
									${enemies.map(enemy => {
										const damagePercent = this.metrics.totalDamage > 0 
							...
							<table>
								...
								<tbody>
									${partyMembers.map(member => {
										const healPercent = this.metrics.totalHealing > 0 
										...
										return html`
											<tr>
												...
												<td>${createProgressBar(healPercent, member.totalHealing, Math.floor(member.totalHealing))}</td>
				...
				<section>
					...
					<dl>
						...
						<dt title="Ratio of healing output to incoming damage - values over 1.0 mean you're keeping up">HPS/DPS ratio:</dt>
