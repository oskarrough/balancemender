...
export class SpellCast {
	...
	static whyNotCast(player: Player, SpellClass?: SpellClass, target = player.getTarget()): CastFailure | undefined {
		...
		if (this.cooldownRemaining(player, SpellClass) > 0) return 'cooldown'
	...
	static cooldownRemaining(player: Player, SpellClass: SpellClass): number {
		const until = player.cooldowns.get(SpellClass.name)
		if (until === undefined) return 0
		return Math.max(0, until - (player.root as GameLoop).elapsedTime)
	}
	...
	static finish(spell: Spell) {
		...
		if (completed) {
			...
			if (spell.cooldown) player.cooldowns.set(spell.name, gameLoop.elapsedTime + spell.cooldown)
		} else {
			logCombat({
				timestamp: Date.now(),
