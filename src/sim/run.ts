...
export class SimLoop extends GameLoop {
	/** Advance every task to `time` (ms since the fight started). */
	runFrame(time: number) {
		;(this as unknown as {_runTasks(t: number): void})._runTasks(time)
	}
}

// `_requestNextFrame` is private to vroum's Loop, so it gets replaced on the prototype
// rather than overridden — the effect is the same: no frame is ever requested.
